...
//                }
        ...
        .onDisappear(perform: {
            let composicion = Composicion(context: viewContext)
            composicion.id_composicion = UUID()
            composicion.timestamp = Date()
            composicion.tipo = tipo_composicion
            composicion.productos = NSOrderedSet(array: selectedItems)
            composicion.num_productos = Int16(selectedItems.count)
